...
AC_DEFUN([AX_LUA_HEADERS],
  ...
  AC_CHECK_HEADER([lua.h], [
  	AC_DEFINE(HAVE_LUA_H, 1, [lua.h exists])
	HAVE_LUA_H=1
	],[
	HAVE_LUA_H=])
  AC_CHECK_HEADER([lualib.h], [
  	AC_DEFINE(HAVE_LUALIB_H, 1, [lualib.h exists])
	HAVE_LUALIB_H=1
	],[
	HAVE_LUALIB_H=])
  CPPFLAGS="$LUA_OLD_CPPFLAGS"
  LUA_INCLUDE+=$LUA_BASE
  AC_SUBST(LUA_INCLUDE)])dnl

AC_DEFUN([AX_LUA_LIBS],
  [_AX_LUA_OPTS
  LUA_BASE=`AS_DIRNAME([$LUA])`
  LUA_BASE="-L$LUA_BASE/../lib"
  LUA_OLD_LIBS="$LIBS"
  ...
  AC_CHECK_LIB([lua$LUA_VERSION],
    ...
    LIBS="$LUA_OLD_LIBS"
    LUA_LIB="$LUA_BASE $LUA_LIB"
    AC_SUBST(LUA_LIB)])dnl
